...
// If the carousel has a data-images-dir attribute, we fetch that folder's
// `index.json` (generated by scripts/build-manifest.js) and build the slides
// dynamically — so adding a new photo just means dropping it into the folder
// and re-running the manifest script. Otherwise we fall back to any
// hard-coded <img class="carousel-img"> children.
async function setupHeroCarousel() {
  const carousel = document.getElementById("hero-carousel");
  if (!carousel) return;

  const dir = carousel.getAttribute("data-images-dir");
  if (dir) {
    try {
      const res = await fetch(`${dir}/index.json`, { cache: "no-store" });
      if (res.ok) {
        const files = await res.json();
        const alt = carousel.getAttribute("data-alt") || "";
        const dotsWrap = carousel.querySelector("#hero-carousel-dots");
        const frag = document.createDocumentFragment();
        files.forEach((name, i) => {
          const img = document.createElement("img");
          img.className = "carousel-img" + (i === 0 ? " is-active" : "");
          img.src = `${dir}/${name}`;
          img.alt = alt;
          img.loading = i === 0 ? "eager" : "lazy";
          frag.appendChild(img);
        });
        // Insert images before the dots container so dots stay at the end.
        carousel.insertBefore(frag, dotsWrap);
      }
    } catch (err) {
      console.warn("Hero carousel manifest load failed:", err);
    }
  }
